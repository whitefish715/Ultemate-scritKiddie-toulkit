...
print('''__   __ _    ____ ____   ___    _  _____ ____  ____ ___ _____ 
\ \ / // \  / ___/ ___| / _ \  | |/ /_ _|  _ \|  _ \_ _| ____|
 \ V // _ \ \___ \___ \| | | | | ' / | || | | | | | | ||  _|  
  | |/ ___ \ ___) |__) | |_| | | . \ | || |_| | |_| | || |___ 
  |_/_/   \_\____/____/ \___/  |_|\_\___|____/|____/___|_____|
                                                      
                                                 \n''')
print('**************************************RAMADAN EDITION*********************')
print('**************************************************************************')
print('***************************************************************by yasso*****\n')
q=input('                       PRESS [y]\n')
...
def fish():
    ...
    print('*************************************************\n')
    h=input('                 the script will install a powerfull phishing tool. ALLOW? [y/n]')   
    if h=='n':
        exit()

    if h=='y':
        d=input('ARE YOU USING LINUX OR TERMUX app ? [l/t]')
        if d=='t':
...
def wifi():
    print('for this u nedd a pc ')
    p=input('**********DO U HAVE A PC [y/n]?****************')
    if p=='y':
        print('********XD*******')
        print('////////////DONT FORGET TO ENABLE MONITOR MODE AFTER FINISHING//////////')
        print('        comand : sudo airmon-ng stop wlan0mon         ')
        k=input('        OK ? [o/n]      ')
        if k=='o':
            pass
        if k=='n':
            exit()
    if p=='n':
        print("*******SORRY U CANT USE THE HACKING TOOL WITHOUT A PC************")
        exit()
    l=input('do u have wifite or u want to installed ?[h/i]')
    if l=='h':
...
def nmaplib():
    ...
    print('')
    print('')
    b=input(' DO U HAVE NMAP OR U WANT TO INSTALL IT [h/i]?')
    if b=='i':
        os.system('sudo apt-get install nmap && sudo apt-get install arp-scan')
    if b=='h':
        print('')
        print('**************XD CONTINUE ***************')
    
    p=input('[1]- ARP scan(discover devices on network)\n[2]- Agressive scan + open ports\n[3]- Intense scan \n[4]- OS discover\n[5]- IP protocol scan\n[6]- Ping scan (identify live hosts)\n[7]- Advanced scan ')
    if p=='1':
...
def vulnscan():
    print('***********************welcome HACKER ******************************************\n')
    print('********************Vulnrebility scanner****************************************')
    os.system('git clone https://github.com/vulnersCom/nmap-vulners.git')
    ip=input(b'==> ENTER THE TARGET IP : ')
    f='sudo nmap -v --script vulners  '
    s=f+ip
    os.system(s)
 
def dos():
     # isclaimer: This script is for educational purposes only.  Do not use against any network that you don't own or have authorization to test.
...
def yasso_main():
    n=(input('[0]- EXIT \n[1]- network scanner\n[2]- vulnerability detection\n[3]- Exploit(msf)\n[4]- Wifi attack\n[5]- Denial of service attack\n[6]- Phishing attack\n[!]-enter a number Professional H4ck3r ("/) :'))
    if n=='1':
        nmaplib()
    if n=='2':
        vulnscan()
